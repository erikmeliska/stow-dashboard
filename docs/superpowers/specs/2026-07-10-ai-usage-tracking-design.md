...
- **`data/usage-cache.json` + `data/usage.json`** — both gitignored (user data), both atomic writes. Deliberately SEPARATE from `data/projects_metadata.jsonl`: no interaction with scan/analyze write paths, no merge discipline needed, scanner untouched; a full scan/sync can never lose usage data.
- **The cache is a durable LEDGER, not a disposable cache.** It stores TOKEN COUNTS (never dollars — prices are applied fresh at aggregation time, so a pricing-table update reprices all history). When a session transcript disappears from disk (user prunes `~/.claude`/`~/.codex`), its ledger entry is KEPT and marked `missing: true` (skipped by stat, still aggregated) — accumulated per-project usage survives cache deletion indefinitely. `Rebuild AI usage` re-parses files that exist and PRESERVES `missing` (ghost) entries; a destructive from-zero rebuild means deleting `data/usage-cache.json` by hand.
