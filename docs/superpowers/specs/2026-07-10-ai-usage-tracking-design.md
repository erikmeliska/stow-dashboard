...
- **UI:**
  ...
  - Details sheet section **AI Usage**: sessions count, active hours, token breakdown (in/out/cache R/W), cost per tool — Claude (per model rows) and Codex (⚠️ estimate) — plus `lastActivity` as TimeAgo and the list-price disclaimer line.
